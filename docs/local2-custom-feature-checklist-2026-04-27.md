...
| ID | 功能 | 必须保留的口径 |
|---|---|---|
| `L2-F0` | local2 当前显示版本 | 当前 Codex 展示版本为 `0.128.0-local2`；该值不是固定写死的 release 文案，而是随当前 Codex 包版本动态变化，显示口径为 `<Codex 版本>-local2`。后续合并到更高 upstream 版本时，应继续保留 `-local2` 本地构建身份后缀。 |
