...
3. 远端模型主链失败后的自动重试覆盖所有会直接终止一次 Codex 对话、compact 或 realtime 模型链路的远端/模型请求错误，不能有错误类型豁免，保持更耐用、更少打断的体验；原来只有部分远端错误会按白名单重试，修改后所有由模型服务主链、Responses HTTP、SSE、WebSocket、compact、realtime/WebSocket 或其他参与一次 assistant turn / compaction 的 Codex 请求链路返回的错误都进入普通自动重试，任何旧 `is_retryable=false`、状态码、错误码、协议层映射或错误分类都不能绕过 retry；`Selected model is at capacity. Please try a different model.`、context window、usage/quota、policy 等服务端返回错误也必须按同一 retry budget 处理；本项强制范围不包含模型目录刷新 `/models` 与 MCP OpenAI 文件上传/下载 URL 生成等旁路 HTTP 辅助请求，这些旁路可以保留自身产品口径，不作为本清单 retry / sticky-break 漏改判定，但如果它们发送 provider token 或展示用户可见错误，仍按对应的鉴权隔离和提示规则验收；所有自动 retry 的每次等待间隔都必须固定为 `5s`，不是“最高 5s”、不是指数退避、不是 jitter/base_delay 逐步增长，也不是 cap-only，包括 Responses HTTP request retry、stream/WebSocket retry、compact retry、realtime/WebSocket 连接 retry 和服务端 `Retry-After` 建议等待都统一等 `5s`；禁止留下 `8s`、`<=5s`、`最高 5s`、退避/抖动/cap-only 的实现、测试名、断言或文档口径；中间失败不写入历史，只保留可见重连提示和可诊断信息，这样用户在模型容量、临时鉴权、网络、服务抖动或其他远端异常时更少看到终态失败，也不会被越来越长的本地重试等待卡住。
   ...
   - 10 分钟自动重试止损：一次用户/agent turn、compact 或 realtime 连接首次进入自动 retry 时开始共享 `600s` 总时钟；普通生成、正常思考和首次请求尚未失败的时间不计入该时钟。Responses HTTP request retry、SSE/WebSocket reconnect、local compact、remote compact、remote compaction v2、realtime/WebSocket 与 WebRTC sideband retry、每次固定 `5s` 等待、fallback transport、provider runtime refresh 和每 3 次 sticky-break 都必须消耗同一个剩余时间，任何内部重置、换路或传输切换不得重新给满 `600s`。剩余时间耗尽时必须取消正在等待或仍卡住的重试请求，停止本轮所有后续自动 retry，发送终态而非 transient 状态，并明确提示“网络重连已持续 10 分钟，已自动停止。请手动重试。”；只有用户重新提交一条新请求才获得新的 `600s` 重试预算。retry 次数配置即使未设上限，用户可见状态也必须显示 `(10 min limit)`，不得继续显示误导性的 `(unbounded)`。这样用户仍可获得短暂网络抖动的自动恢复，但不会因无界次数配置等待二十分钟以上。
...
| retry 10 分钟止损 | ordinary sampling、Responses HTTP request retry、SSE/WebSocket reconnect、local compact、old remote compact、remote compaction v2、realtime/WebSocket、WebRTC sideband、fallback transport、provider runtime refresh、sticky-break | 首次自动 retry 开始共享 `600s` 总预算；普通生成时间不计入；每个 retry 子路径、5 秒等待和内部恢复都消耗同一预算且不能重置；到时取消当前等待或请求、停止自动 retry、发出“网络重连已持续 10 分钟，已自动停止。请手动重试。”终态；重试次数未设上限时状态显示 `(10 min limit)` 而非 `(unbounded)`；重新提交新请求才重新计时 |
...
- 无界次数配置不能把内部哨兵值显示给用户；`u64::MAX` 只代表“不设次数上限”，用户界面禁止出现 `18446744073709551615`，且在当前 10 分钟总时限下必须显示 `(10 min limit)`，不能显示 `(unbounded)`。否则用户会把内部实现数字或错误的“无限等待”理解成异常状态。
...
- 用户报告的 `503 retry N (unbounded)` 必须迁移为“未设次数上限但共享 600 秒总时限”的场景覆盖；不写 `request_max_retries` 仍代表次数不封顶，但 release exe 必须在 10 分钟时自动终止，不能只用有界次数 retry 或无限等待替代。
...
- 用户可见 retry 序号必须和内部 route recovery 局部计数分离；第 3、6、9 次 sticky-break 可以重启 HTTP 请求、重置 WebSocket session 或刷新局部 fallback 计数，但 `503 retry N (10 min limit)` / `Reconnecting... N (10 min limit)` 的 N 必须继续累计为 4、5、6，不能回到 0 或 1。
