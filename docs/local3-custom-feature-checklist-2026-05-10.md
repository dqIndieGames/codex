...
3. 远端模型主链失败后的自动重试覆盖所有会直接终止一次 Codex 对话、compact 或 realtime 模型链路的远端/模型请求错误，不能有错误类型豁免，保持更耐用、更少打断的体验；原来只有部分远端错误会按白名单重试，修改后所有由模型服务主链、Responses HTTP、SSE、WebSocket、compact、realtime/WebSocket 或其他参与一次 assistant turn / compaction 的 Codex 请求链路返回的错误都进入普通自动重试，任何旧 `is_retryable=false`、状态码、错误码、协议层映射或错误分类都不能绕过 retry；`Selected model is at capacity. Please try a different model.`、context window、usage/quota、policy 等服务端返回错误也必须按同一 retry budget 处理；本项强制范围不包含模型目录刷新 `/models` 与 MCP OpenAI 文件上传/下载 URL 生成等旁路 HTTP 辅助请求，这些旁路可以保留自身产品口径，不作为本清单 retry / sticky-break 漏改判定，但如果它们发送 provider token 或展示用户可见错误，仍按对应的鉴权隔离和提示规则验收；所有自动 retry 的每次等待间隔都必须固定为 `5s`，不是“最高 5s”、不是指数退避、不是 jitter/base_delay 逐步增长，也不是 cap-only，包括 Responses HTTP request retry、stream/WebSocket retry、compact retry、realtime/WebSocket 连接 retry 和服务端 `Retry-After` 建议等待都统一等 `5s`；禁止留下 `8s`、`<=5s`、`最高 5s`、退避/抖动/cap-only 的实现、测试名、断言或文档口径；中间失败不写入历史，只保留可见重连提示和可诊断信息，这样用户在模型容量、临时鉴权、网络、服务抖动或其他远端异常时更少看到终态失败，也不会被越来越长的本地重试等待卡住。
   ...
   - 10 分钟自动重试 watchdog（看门狗：超过时限时主动打断卡住的那一次请求，再继续恢复）：一次用户/agent turn、compact 或 realtime 连接首次进入自动 retry 时开始共享 `600s` 观察窗口；普通生成、正常思考和首次请求尚未失败的时间不计入该窗口。Responses HTTP request retry、SSE/WebSocket reconnect、local compact、remote compact、remote compaction v2、realtime/WebSocket 与 WebRTC sideband retry、每次固定 `5s` 等待、fallback transport、provider runtime refresh 和每 3 次 sticky-break 都消耗同一个窗口，任何内部重置、换路或传输切换不得悄悄跳过它。窗口到期时必须取消正在等待或仍卡住的当前尝试，发出 transient（临时状态：本轮仍在继续，不能当作最终失败）提示“网络重连已持续 10 分钟，已自动中断当前尝试，正在自动重试。”，然后沿同一用户/agent turn 的自动 retry 链继续；不得要求用户手动提交新请求，不得把到期变成终态。自动 retry 的次数配置可保持未设上限或非常大；用户可见状态显示持续累计的序号和 `(auto retry)`，不得显示内部 `u64::MAX` 哨兵值或过时的 `(10 min limit)` / `(unbounded)`。这样用户仍可获得卡住请求被主动打断的恢复机会，同时保留持续自动重试。
...
| retry 10 分钟 watchdog | ordinary sampling、Responses HTTP request retry、SSE/WebSocket reconnect、local compact、old remote compact、remote compaction v2、realtime/WebSocket、WebRTC sideband、fallback transport、provider runtime refresh、sticky-break | 首次自动 retry 开始共享 `600s` 观察窗口；普通生成时间不计入；每个 retry 子路径、5 秒等待和内部恢复都消耗同一窗口且不能绕过；到时取消当前等待或请求、发出“网络重连已持续 10 分钟，已自动中断当前尝试，正在自动重试。”临时状态、沿同一 turn 自动 retry，不得终态或要求手动重试；无上限或很大次数时状态显示累计序号与 `(auto retry)`，不显示 `(10 min limit)` / `(unbounded)` |
...
- 用户报告的 `503 retry N (unbounded)` 必须迁移为“未设次数上限或非常大次数、共享 600 秒 watchdog 窗口”的场景覆盖；不写 `request_max_retries` 仍代表次数不封顶，但 release exe 必须在每次 10 分钟窗口到期时自动中断卡住尝试并继续 retry，不能终止 turn 或要求用户手动重试。
