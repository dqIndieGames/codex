...
> 2026-08-07 当前口径：模型主链的 HTTP、SSE、Responses WebSocket、compact、realtime/WebSocket 与 WebRTC sideband 每次尝试最多允许连续 `300s` 无网络进展，随后中断当前尝试并按既有固定 `5s` 间隔自动重试；provider 显式连接/空闲超时也不得超过 `300s`，原本短于 `300s` 的超时继续保留。下文历史段落中的 `10 分钟` / `600s` 口径均由本条替代。
...
3. 远端模型主链失败后的自动重试覆盖所有会直接终止一次 Codex 对话、compact 或 realtime 模型链路的远端/模型请求错误，不能有错误类型豁免，保持更耐用、更少打断的体验；原来只有部分远端错误会按白名单重试，修改后所有由模型服务主链、Responses HTTP、SSE、WebSocket、compact、realtime/WebSocket 或其他参与一次 assistant turn / compaction 的 Codex 请求链路返回的错误都进入普通自动重试，任何旧 `is_retryable=false`、状态码、错误码、协议层映射或错误分类都不能绕过 retry；`Selected model is at capacity. Please try a different model.`、context window、usage/quota、policy 等服务端返回错误也必须按同一 retry budget 处理；本项强制范围不包含模型目录刷新 `/models` 与 MCP OpenAI 文件上传/下载 URL 生成等旁路 HTTP 辅助请求，这些旁路可以保留自身产品口径，不作为本清单 retry / sticky-break 漏改判定，但如果它们发送 provider token 或展示用户可见错误，仍按对应的鉴权隔离和提示规则验收；所有自动 retry 的每次等待间隔都必须固定为 `5s`，不是“最高 5s”、不是指数退避、不是 jitter/base_delay 逐步增长，也不是 cap-only，包括 Responses HTTP request retry、stream/WebSocket retry、compact retry、realtime/WebSocket 连接 retry 和服务端 `Retry-After` 建议等待都统一等 `5s`；禁止留下 `8s`、`<=5s`、`最高 5s`、退避/抖动/cap-only 的实现、测试名、断言或文档口径；中间失败不写入历史，只保留可见重连提示和可诊断信息，这样用户在模型容量、临时鉴权、网络、服务抖动或其他远端异常时更少看到终态失败，也不会被越来越长的本地重试等待卡住。
   ...
   - context window 溢出后的图片自愈（只改图，不改 `previous_response_id`）：同一轮 sampling 连续溢出时，第 1、2 次原样重试；第 3、6、9、12 次才升档。第一档最后 5 张保持原图、其余 `original→high`；第二档最后 1 张保持原图、其余降档；第三档只留最后 5 张真图、更早换成 1×1；第四档只留最后 1 张。某档 0 变化则同一次触发连升。改动同时作用于内存历史和本地会话记录：追加当前窗口的 `Compacted.replacement_history` 快照（不推进 compact 窗口、不发 compact UI），使 resume 重建出瘦身后的图而不是原图。不改磁盘上的截图文件。sticky-break 与 `previous_response_id` 仍由既有每 3 次逻辑自行管理，图片梯子不得再 drop id 或重置 WebSocket。

   - 10 分钟单次请求卡顿 watchdog（看门狗：当前这一次请求连续无网络进展超过时限时，主动打断它并继续恢复）：每一次 Responses HTTP request（包括首次请求与后续每一次 retry）发出时都独立获得完整 `600s`；前一次请求已经等待多久、固定 `5s` retry 等待、fallback transport、provider runtime refresh、每 3 次 sticky-break、compact 或 realtime 的其他尝试，都不得扣减下一次请求的 `600s`。HTTP 尚未返回响应头时，从本次发送开始计时；SSE/WebSocket 已建立后，以当前流连续没有新网络事件的时间计时，收到新事件即重新计算本次空闲时间，不能把仍在持续输出的正常长生成按总时长误杀；已有更短且显式配置的连接/空闲超时可以更早触发，但同样只作用于当前尝试。当前尝试连续卡满 10 分钟时必须取消该请求或连接，发出 transient（临时状态：本轮仍在继续，不能当作最终失败）提示“本次网络请求连续 10 分钟无进展，已自动中断，正在自动重试。”，随后按既有固定 `5s` 间隔进入下一次自动 retry；下一次请求重新获得完整 `600s`，不得要求用户手动提交新请求，也不得把超时变成 turn 终态。自动 retry 次数可保持未设上限或非常大；用户可见状态继续显示累计序号和 `(auto retry)`，不得显示内部 `u64::MAX` 哨兵值或过时的 `(10 min limit)` / `(unbounded)`。
...
| retry 10 分钟单次卡顿 watchdog | ordinary sampling、首次 Responses HTTP、Responses HTTP request retry、SSE/WebSocket reconnect、local compact、old remote compact、remote compaction v2、realtime/WebSocket、WebRTC sideband、fallback transport、provider runtime refresh、sticky-break | 每一次 HTTP/连接尝试独立获得完整 `600s`；首次请求也受保护；前序请求、5 秒等待和内部恢复不扣减下一次额度；SSE/WebSocket 按连续无事件时间判断卡顿，收到进展重新计空闲时间；卡满时只取消当前尝试、发出“本次网络请求连续 10 分钟无进展，已自动中断，正在自动重试。”临时状态并固定等待 5 秒后重连，下一次重新获得完整 `600s`，不得终态或要求手动重试；无上限或很大次数时状态显示累计序号与 `(auto retry)`，不显示 `(10 min limit)` / `(unbounded)` |
...
- 无界次数配置不能把内部哨兵值显示给用户；`u64::MAX` 只代表“不设次数上限”，用户界面禁止出现 `18446744073709551615`，应显示持续累计序号与 `(auto retry)`，不能显示 `(unbounded)` 或已失效的 `(10 min limit)`。10 分钟 watchdog 只打断当前卡住尝试后继续自动 retry，不是次数上限。
...
- 用户报告的 `503 retry N (unbounded)` 必须迁移为“未设次数上限或非常大次数、每一次请求各自拥有完整 600 秒卡顿 watchdog”的场景覆盖；不写 `request_max_retries` 仍代表次数不封顶，release exe 必须在某一次请求连续卡满 10 分钟时只中断该次尝试并继续 retry，下一次不得继承或扣减前一次已经消耗的时间，不能终止 turn 或要求用户手动重试。
...
- 用户可见 retry 序号必须和内部 route recovery 局部计数分离；第 3、6、9 次 sticky-break、10 分钟 watchdog 中断当前尝试、重启 HTTP 请求、重置 WebSocket session 或刷新局部 fallback 计数，都不得把 `503 retry N (auto retry)` / `Reconnecting... N (auto retry)` 的 N 重置；N 必须继续累计为 4、5、6，不能回到 0 或 1。
