...
- 基于当前仓库静态源码/脚本/文档与前序对话，先把你的个人定制目标明确为六条主线：`local1` 显示链、`Responses` 主链重试增强链、Provider runtime 热刷新链、跨 Provider 历史发现链、`gpt-5.4 priority` 请求层兜底链、Windows/TUI 默认日志降噪链。
...
| `F13` | `保持不变` | `121` 不改变 `force_gpt54_priority_fallback` 顶层开关与 `priority/Fast/Flex` 现有口径。 |
...
| `A2` | `保持不变` | `121` 不改变 `force_gpt54_priority_fallback` 仅允许顶层 `config.toml` 生效的归档口径。 |
