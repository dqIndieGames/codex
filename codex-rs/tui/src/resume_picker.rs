...
async fn run_session_picker_with_loader(
    ...
    let provider_filter = default_provider_filter_for_picker(config, action, is_remote);
...
fn rollout_provider_filter_and_fallback(
    ...
    match provider_filter {
        ProviderFilter::Any => (None, config.model_provider_id.to_string()),
        ProviderFilter::MatchDefault(default_provider) => {
            let fallback_provider = default_provider.clone();
            (Some(default_provider), fallback_provider)
        }
    }
}
...
fn spawn_rollout_page_loader(
    ...
    Arc::new(move |request: PageLoadRequest| {
        ...
        tokio::spawn(async move {
            let (provider_filter, fallback_provider) =
                rollout_provider_filter_and_fallback(&config, request.provider_filter);
...
mod tests {
    ...
    #[tokio::test]
    async fn picker_provider_filter_uses_all_providers_for_local_resume() {
        let temp_dir = TempDir::new().expect("tempdir");
        let config = build_config(&temp_dir).await;

        let provider_filter = default_provider_filter_for_picker(
            &config,
            SessionPickerAction::Resume,
            /*is_remote*/ false,
        );

        assert_eq!(provider_filter, ProviderFilter::Any);
    }

    #[tokio::test]
    async fn picker_provider_filter_keeps_current_provider_for_local_fork() {
        let temp_dir = TempDir::new().expect("tempdir");
        let config = build_config(&temp_dir).await;
        ...
        assert_eq!(
            provider_filter,
            ProviderFilter::MatchDefault(config.model_provider_id)
        );
    }

    #[tokio::test]
    async fn picker_provider_filter_uses_all_providers_for_remote_sessions() {
        let temp_dir = TempDir::new().expect("tempdir");
        let config = build_config(&temp_dir).await;

        let resume_filter = default_provider_filter_for_picker(
            &config,
            SessionPickerAction::Resume,
            /*is_remote*/ true,
        );
        let fork_filter = default_provider_filter_for_picker(
            &config,
            SessionPickerAction::Fork,
            /*is_remote*/ true,
        );

        assert_eq!(resume_filter, ProviderFilter::Any);
        assert_eq!(fork_filter, ProviderFilter::Any);
    }

    #[tokio::test]
    async fn rollout_provider_config_keeps_current_provider_as_any_fallback() {
        let temp_dir = TempDir::new().expect("tempdir");
        let config = build_config(&temp_dir).await;

        let (provider_filter, fallback_provider) =
            rollout_provider_filter_and_fallback(&config, ProviderFilter::Any);

        assert_eq!(provider_filter, None);
        assert_eq!(fallback_provider, config.model_provider_id);
    }

    #[tokio::test]
    async fn rollout_provider_config_keeps_match_default_for_filter_and_fallback() {
        let temp_dir = TempDir::new().expect("tempdir");
        let config = build_config(&temp_dir).await;

        let (provider_filter, fallback_provider) = rollout_provider_filter_and_fallback(
            &config,
            ProviderFilter::MatchDefault(String::from("openai")),
        );

        assert_eq!(provider_filter, Some(String::from("openai")));
        assert_eq!(fallback_provider, String::from("openai"));
    }
