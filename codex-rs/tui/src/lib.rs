...
fn latest_session_lookup_params(
    ...
    ThreadListParams {
        ...
        sort_key: Some(AppServerThreadSortKey::UpdatedAt),
        sort_direction: None,
        model_providers: match (is_remote, provider_filter) {
            (true, _) | (false, LatestSessionProviderFilter::Any) => None,
            (false, LatestSessionProviderFilter::MatchCurrent) => {
...
pub async fn run_main(
    ...
    let feedback_layer = runtime_load_gates.feedback.then(|| feedback.logger_layer());
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            match lookup_latest_session_target_with_app_server(
                ...
                LatestSessionProviderFilter::Any,
            )
            .await?
            {
                Some(target_session) => resume_picker::SessionSelection::Fork(target_session),
...
mod tests {
    ...
    #[tokio::test]
    async fn latest_session_lookup_params_use_all_providers_for_embedded_resume()
    -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
    ...
    #[tokio::test]
    async fn latest_session_lookup_params_use_all_providers_for_embedded_fork()
    -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let config = build_config(&temp_dir).await?;
        let cwd = temp_dir.path().join("project");
