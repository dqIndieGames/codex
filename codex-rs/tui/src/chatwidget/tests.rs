...
async fn replayed_retryable_app_server_error_keeps_turn_running() {
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                message: "Reconnecting... 1".to_string(),
                codex_error_info: None,
...
#[tokio::test]
async fn stream_error_429_retry_updates_status_indicator() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.bottom_pane.set_task_running(/*running*/ true);
    let msg = "429 retry 1";
    ...
    assert!(
        cells.is_empty(),
        "expected no history cell for 429 retry StreamError event"
    );
    let status = chat
        .bottom_pane
        .status_widget()
...
#[tokio::test]
async fn stream_error_402_retry_updates_status_indicator() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.bottom_pane.set_task_running(/*running*/ true);
    let msg = "402 retry 1";
    ...
    assert!(
        cells.is_empty(),
        "expected no history cell for 402 retry StreamError event"
    );
    let status = chat
        .bottom_pane
        .status_widget()
...
async fn replayed_stream_error_does_not_set_retry_status_or_status_indicator() {
    ...
    chat.replay_initial_messages(vec![EventMsg::StreamError(StreamErrorEvent {
        message: "Reconnecting... 2".to_string(),
        codex_error_info: Some(CodexErrorInfo::Other),
        additional_details: Some("Idle timeout waiting for SSE".to_string()),
    })]);
...
#[tokio::test]
async fn status_line_codex_version_uses_display_version() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.tui_status_line = Some(vec!["codex-version".to_string()]);

    chat.refresh_status_surfaces();

    assert_eq!(
        chat.status_line_value_for_item(&StatusLineItem::CodexVersion),
        Some(CODEX_CLI_DISPLAY_VERSION.to_string())
    );
    assert_eq!(
...
#[tokio::test]
async fn placeholder_session_header_uses_display_version() {
    ...
    assert!(rendered.contains(&format!("(v{CODEX_CLI_DISPLAY_VERSION})")));
}
