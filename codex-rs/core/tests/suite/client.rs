...
fn retry_route_recovery_provider(
    server: &MockServer,
...
fn sse_response_template(body: String) -> ResponseTemplate {
    ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .set_body_raw(body, "text/event-stream")
}

fn prompt_cache_keys(requests: &[ResponsesRequest]) -> Vec<String> {
    requests
        .iter()
        .map(|request| {
...
async fn function_call_output_request_retry_keeps_tool_output_and_route_recovers() {
    ...
    let server = MockServer::start().await;
    let call_id = "call-retry-tool-output";
    let first_response = sse(vec![
        ev_response_created("resp-tool-call"),
        ev_function_call(call_id, "test_tool", "{}"),
        ev_completed("resp-tool-call"),
    ]);
    let final_response = sse(vec![
        ev_assistant_message("msg-after-tool", "done"),
        ev_completed("resp-after-tool"),
    ]);
    let response_mock = mount_response_sequence(
        &server,
        vec![
            ...
            ResponseTemplate::new(502),
            ResponseTemplate::new(504),
            sse_response_template(final_response),
        ],
    )
    .await;
    ...
    let all_cache_keys = prompt_cache_keys(&requests);
    let tool_output_requests = requests
        .iter()
        .filter(|request| !request.inputs_of_type("function_call_output").is_empty())
        .cloned()
        .collect::<Vec<_>>();
