...
// local3: Retry-After / rate-limit message delays must not change the fixed 5s wait.
/// Remote compaction v2 retries stay on the local3 fixed 5s interval even when the
/// rate-limit body embeds a shorter retry suggestion.
#[tokio::test(flavor = "current_thread")]
async fn compact_v2_rate_limit_message_keeps_fixed_five_second_retry_delay() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(wait_for_retry(&mut telemetry, &retry).await >= Duration::from_secs(5));
    wait_for_turn_completion(&test).await;
...
/// Remote compaction rate-limit messages still wait a fixed 5s without an HTTP header.
#[tokio::test(flavor = "current_thread")]
async fn compact_v2_rate_limit_message_without_retry_after_keeps_fixed_five_second_delay() -> Result<()>
{
    skip_if_no_network!(Ok(()));
...
/// Rate-limit messages still wait a fixed 5s without an HTTP header.
#[tokio::test(flavor = "current_thread")]
async fn sse_rate_limit_message_keeps_fixed_five_second_retry_delay() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
// local3: Retry-After headers must not change the fixed 5s wait.
/// Rate-limit message delays and Retry-After headers both stay on the local3 5s interval.
#[tokio::test(flavor = "current_thread")]
async fn sse_rate_limit_message_with_retry_after_keeps_fixed_five_second_retry_delay() -> Result<()> {
    skip_if_no_network!(Ok(()));
