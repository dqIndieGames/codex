...
pub(crate) struct Session {
    ...
    pending_provider_runtime_refresh: Mutex<Option<PendingProviderRuntimeRefresh>>,
...
impl Session {
    ...
    pub(crate) async fn reload_user_config_layer(&self) {
        if let Err(err) = self.reload_user_config_layer_result().await {
            warn!("{err}");
        }
    }
    ...
    fn read_user_config_toml_value(config_toml_path: &AbsolutePathBuf) -> CodexResult<TomlValue> {
        match std::fs::read_to_string(config_toml_path.as_path()) {
            Ok(contents) => toml::from_str::<TomlValue>(&contents).map_err(|err| {
                CodexErr::InvalidRequest(format!(
                    "failed to parse user config while reloading layer: {err}"
                ))
            }),
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                Ok(TomlValue::Table(Default::default()))
            }
            Err(err) => Err(CodexErr::Io(err)),
        }
    }

    async fn reload_user_config_layer_result(&self) -> CodexResult<()> {
        let (config, config_toml_path) = {
            let state = self.state.lock().await;
            let config = Arc::clone(&state.session_configuration.original_config_do_not_use);
            let config_toml_path = Self::resolve_user_config_toml_path(config.as_ref())?;
    ...
    async fn apply_provider_runtime_refresh(
        ...
        self.services
            .model_client
            .refresh_provider_runtime(base_url, experimental_bearer_token);
        Ok(())
    }
...
struct PendingProviderRuntimeRefresh {
