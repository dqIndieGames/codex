...
use crate::codex_thread::ProviderRuntimeRefreshAllLoadedReport;
use crate::codex_thread::ProviderRuntimeRefreshFailure;
...
impl ThreadManager {
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                thread_created_tx,
                thread_id_generator: default_thread_id_generator(),
                models_manager: std::sync::Mutex::new(
    ...
    pub fn get_models_manager(&self) -> SharedModelsManager {
        self.state.cloned_models_manager()
    }
    ...
    pub async fn refresh_models_manager_from_config(&self, config: &Config) {
        ...
        models_manager
            ...
            .await;
        self.state.replace_models_manager(models_manager);
    }
    ...
    pub async fn list_models(
        ...
        self.state
            .cloned_models_manager()
            .list_models(refresh_strategy, http_client_factory)
            .await
    }

    pub fn list_collaboration_modes(&self) -> Vec<CollaborationModeMask> {
        self.state.cloned_models_manager().list_collaboration_modes()
    }
    ...
    pub async fn refresh_loaded_provider_runtime(
        ...
        for (thread_id, thread) in threads {
            match thread.refresh_provider_runtime().await {
                Ok(ProviderRuntimeRefreshStatus::Applied) => {
                    report.applied_thread_ids.push(thread_id);
                }
                Ok(ProviderRuntimeRefreshStatus::Queued) => {
                    report.queued_thread_ids.push(thread_id);
                }
                Err(err) => {
                    report.failed_threads.push(ProviderRuntimeRefreshFailure {
                        thread_id,
...
impl ThreadManagerState {
    fn cloned_models_manager(&self) -> SharedModelsManager {
        self.models_manager
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    fn replace_models_manager(&self, models_manager: SharedModelsManager) {
        *self
            .models_manager
            .lock()
