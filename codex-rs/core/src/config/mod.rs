...
pub struct Config {
    ...
    /// When true, all Responses requests force the priority service tier.
    ...
    /// When `true`, app-server batches high-frequency notification deltas while preserving
    /// completion/error flush boundaries.
    /// Defaults to `true`.
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let config = Self {
            ...
            rollout_batch_flush_enabled: cfg
                ...
                .as_ref()
                .and_then(|optimizations| optimizations.rollout_batch_flush)
                .unwrap_or(true),
            app_server_notification_coalescing_enabled: cfg
                .runtime_optimizations
                .as_ref()
                .and_then(|optimizations| optimizations.app_server_notification_coalescing)
                .unwrap_or(true),
            tool_suggest,
