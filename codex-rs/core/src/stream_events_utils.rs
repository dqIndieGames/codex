...
const LOCAL3_CHECKLIST: &str =
    include_str!("../../../docs/local3-custom-feature-checklist-2026-05-10.md");

fn local3_first_turn_checklist_entries() -> Vec<(usize, String)> {
    let entries = LOCAL3_CHECKLIST
        .lines()
        .filter_map(|line| {
            let trimmed = line.trim();
            let (index, text) = trimmed.split_once(". ")?;
            let index = index.parse::<usize>().ok()?;
            (1..=13).contains(&index).then(|| (index, text.to_string()))
        })
        .collect::<Vec<_>>();
    ...
    debug_assert_eq!(
        ...
        "local3 checklist rows must contain numbered items 1..13 in order"
    );
...
pub(crate) fn local1_first_turn_checklist_prefix() -> String {
    ...
    let mut lines = vec![
        format!("local3 定制功能已启用（当前版本：{version}-local3）"),
        String::new(),
    ];
    lines.extend(
        local3_first_turn_checklist_entries()
            .into_iter()
