...
use codex_history::ImagesShrunkItem;
...
impl Session {
    ...
    async fn apply_provider_runtime_refresh(
        ...
        let (previous_config, new_config) = {
            ...
            let previous_config = notify_config_contributors
                .then(|| self.build_effective_session_config(&state.session_configuration));
            state.session_configuration.provider = create_model_provider(
                provider.clone(),
                Some(Arc::clone(&self.services.auth_manager)),
            );
            Arc::make_mut(&mut state.session_configuration.step_settings).service_tier =
                service_tier.clone();
        ...
        self.services
            .model_client
            .refresh_provider_runtime(provider, force_service_tier_priority);
        Ok(())
    }
    ...
    fn read_latest_provider_runtime_refresh(
        ...
        let user_config = resolve_relative_paths_in_config_toml(user_config, config_base_dir.as_path())
            .map_err(|err| {
                CodexErr::InvalidRequest(format!(
        ...
        let merged_toml = config
            ...
            .map_err(|err| {
                CodexErr::InvalidRequest(format!(
                    "failed to apply user config while refreshing provider runtime: {err}"
                ))
            })?
            .effective_config();
        let cfg: ConfigToml =
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        self.maybe_arm_first_turn_checklist_for_regular_input(input)
            .await;
    ...
    pub(crate) fn first_turn_checklist_candidate_source(
    ...
    pub(crate) fn first_turn_checklist_regular_input_matches_trigger(input: &[UserInput]) -> bool {
        matches!(
            input,
            [UserInput::Text {
                text,
