...
use crate::codex_thread::ProviderRuntimeRefreshStatus;
...
impl Session {
    ...
    async fn apply_provider_runtime_refresh(
        &self,
    ...
    fn resolve_provider_runtime_refresh(
        ...
        .map_err(|err| {
            CodexErr::InvalidRequest(format!(
                "failed to resolve feature config while refreshing provider runtime: {err}"
            ))
        })?;
        let fast_mode_enabled = features.enabled(Feature::FastMode);
        let service_tier = cfg.service_tier.and_then(|service_tier| {
            match ServiceTier::from_request_value(&service_tier) {
                Some(ServiceTier::Fast) => {
                    fast_mode_enabled.then(|| ServiceTier::Fast.request_value().to_string())
                }
    ...
    fn read_user_config_toml_value(config_toml_path: &AbsolutePathBuf) -> CodexResult<TomlValue> {
        match std::fs::read_to_string(config_toml_path) {
            Ok(contents) => toml::from_str(&contents).map_err(|err| {
                CodexErr::InvalidRequest(format!(
                    "failed to parse user config `{}` while refreshing provider runtime: {err}",
            ...
            Err(err) => Err(CodexErr::InvalidRequest(format!(
                "failed to read user config `{}` while refreshing provider runtime: {err}",
                config_toml_path.display()
            ))),
        }
    }
    ...
    pub(crate) async fn apply_pending_provider_runtime_refresh_if_requested(&self) {
        let refresh = { self.pending_provider_runtime_refresh.lock().await.take() };
        let Some(refresh) = refresh else {
            return;
        };

        if let Err(err) = self.apply_provider_runtime_refresh(refresh).await {
            warn!("failed to apply pending provider runtime refresh: {err}");
        }
    }
    ...
    pub(crate) async fn refresh_provider_runtime(
        ...
        // Runtime refresh replaces provider transport fields, service-tier
        // routing, and the effective fast-mode state. Apply them immediately so
        // active app-server/TUI/exec sessions stop using stale provider
        // settings without waiting for a new turn.
        {
    ...
    /// and the applied tier is persisted as an `ImagesShrunk` marker so
    /// resume, fork and rollback all rebuild the shrunken images.
    ///
    /// The marker is deliberately not a `Compacted` checkpoint: it carries no
    /// replacement history, does not advance the compaction window and is never
    /// treated as a replay baseline, so turn boundaries and rollback counts stay
    /// intact and the rollout stays small.
    pub(crate) async fn apply_context_overflow_image_ladder(
        ...
        self.persist_rollout_items(&[RolloutItem::ImagesShrunk(ImagesShrunkItem {
            tier: report.tier,
            changed: u32::try_from(report.changed).ok(),
        })])
        .await;
        let _ = self.flush_rollout().await;
        Some(report)
    ...
    pub(crate) async fn maybe_arm_first_turn_checklist_for_regular_input(
        &self,
        input: &[UserInput],
    ) {
        if input.is_empty() {
