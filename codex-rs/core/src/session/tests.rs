...
fn write_provider_refresh_test_config(
    codex_home: &Path,
    base_url: Option<&str>,
...
"#
    ...
    std::fs::write(codex_home.join("config.toml"), config_toml)
        .expect("write provider refresh test config");
}

async fn make_session_and_context_from_config(
    config: Arc<Config>,
    ...
    let session = Arc::new(Session {
        ...
        pending_provider_runtime_refresh: Mutex::new(None),
...
fn first_turn_checklist_regular_input_matches_only_single_trimmed_nihao_text() {
    ...
    assert!(!Session::first_turn_checklist_regular_input_matches_trigger(&[text_input("hello")]));
    assert!(!Session::first_turn_checklist_regular_input_matches_trigger(&[text_input("你好啊")]));
    assert!(!Session::first_turn_checklist_regular_input_matches_trigger(&[text_input("你好。")]));
    assert!(
        !Session::first_turn_checklist_regular_input_matches_trigger(&[text_input("你好\n别的字")])
    );
    assert!(
        !Session::first_turn_checklist_regular_input_matches_trigger(&[
            text_input("你好"),
            text_input("第二条")
        ])
    );
    ...
    assert!(
        !Session::first_turn_checklist_regular_input_matches_trigger(&[UserInput::Text {
            text: "你好".to_string(),
...
#[tokio::test]
async fn refresh_provider_runtime_applies_immediately_for_idle_thread() {
    let codex_home = tempfile::tempdir().expect("create temp dir");
    ...
    let next_turn = session
        ...
        .await;
    assert_eq!(
        next_turn.provider.info().base_url.as_deref(),
        Some("https://new.example.com/v1")
    );
    assert_eq!(
        next_turn
...
#[tokio::test]
async fn refresh_provider_runtime_clears_deleted_fields() {
    let codex_home = tempfile::tempdir().expect("create temp dir");
    write_provider_refresh_test_config(
        codex_home.path(),
        Some("https://old.example.com/v1"),
    ...
    let next_turn = session
        .new_default_turn_with_sub_id("next-turn".to_string())
        .await;
    assert_eq!(next_turn.provider.info().base_url, None);
    assert_eq!(next_turn.provider.info().experimental_bearer_token, None);
}
...
async fn refresh_provider_runtime_fails_when_provider_is_missing() {
    ...
    assert!(
        err.to_string()
            .contains("model provider `mock_provider` no longer exists"),
...
#[tokio::test]
async fn refresh_provider_runtime_fails_without_mutating_state_for_invalid_user_config() {
    let codex_home = tempfile::tempdir().expect("create temp dir");
    write_provider_refresh_test_config(
        codex_home.path(),
        Some("https://old.example.com/v1"),
    ...
    assert!(
        err.to_string().contains("failed to parse user config"),
        "unexpected error: {err}"
    );
...
async fn ensure_task_for_pending_inputs_uses_latest_provider_runtime_after_active_refresh() {
    ...
    *session.active_turn.lock().await = None;
    ...
    let turn_context = session
        .new_default_turn_with_sub_id("wake-turn".to_string())
        .await;
    session
        .spawn_task(
