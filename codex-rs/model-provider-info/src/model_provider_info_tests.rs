...
#[test]
fn explicit_zero_request_retries_disables_request_attempts() {
    let provider = ModelProviderInfo {
        request_max_retries: Some(0),
        ..ModelProviderInfo::create_openai_provider(/*base_url*/ None)
    };

    assert_eq!(provider.request_retry_attempts(), 0);
}

#[test]
fn explicit_zero_stream_retries_disables_stream_attempts() {
    let provider = ModelProviderInfo {
        stream_max_retries: Some(0),
        ..ModelProviderInfo::create_openai_provider(/*base_url*/ None)
    };

    assert_eq!(provider.stream_retry_budget(), Some(0));
}
