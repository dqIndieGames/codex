...
impl RolloutRecorder {
    ...
    pub async fn new(
        ...
        let flush_mode = if config.rollout_batch_flush_enabled() {
            RolloutFlushMode::Batch
        } else {
            RolloutFlushMode::PerLine
        };
...
impl RolloutWriterState {
    fn new(
        ...
        Self {
            writer: file.map(|file| JsonlWriter { file, flush_mode }),
            deferred_log_file_info,
    ...
    async fn flush_if_materialized(&mut self) {
        if self.flush_mode == RolloutFlushMode::Batch {
            return;
        }
        if self.is_deferred() {
            return;
...
pub async fn append_rollout_item_to_path(
    ...
    let mut writer = JsonlWriter {
        file,
        flush_mode: RolloutFlushMode::PerLine,
    };
...
impl JsonlWriter {
    ...
    async fn write_line(&mut self, item: &impl serde::Serialize) -> std::io::Result<()> {
        ...
        if self.flush_mode == RolloutFlushMode::PerLine {
