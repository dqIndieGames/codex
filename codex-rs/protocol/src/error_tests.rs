...
#[test]
fn retry_time_budget_interruption_stays_retryable_and_keeps_its_message() {
    let message = "retry watchdog interrupted the current attempt";
    let err = CodexErr::RetryTimeBudgetInterrupted(message.to_string());

    assert_eq!(err.to_string(), message);
    assert!(err.is_retryable());
    assert!(err.is_retry_time_budget_interrupted());
