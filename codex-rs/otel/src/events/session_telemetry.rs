...
pub trait WebsocketRequestTelemetryContextArg<'a> {
    fn into_parts(self) -> (bool, Option<&'a AgentIdentityTelemetry>);
}

impl<'a> WebsocketRequestTelemetryContextArg<'a> for bool {
    fn into_parts(self) -> (bool, Option<&'a AgentIdentityTelemetry>) {
        (self, None)
    }
}

impl<'a> WebsocketRequestTelemetryContextArg<'a> for Option<&'a AgentIdentityTelemetry> {
    fn into_parts(self) -> (bool, Option<&'a AgentIdentityTelemetry>) {
        (true, self)
    }
}
...
impl SessionTelemetry {
    ...
    pub fn record_websocket_request<'a>(
        &self,
        duration: Duration,
        error: Option<&str>,
        connection_reused: bool,
        context: impl WebsocketRequestTelemetryContextArg<'a>,
    ) {
        let (emit_log_trace, agent_identity_telemetry) = context.into_parts();
