...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_provider_runtime_refresh_all_loaded(
        ...
        Ok(Some(
            ThreadProviderRuntimeRefreshAllLoadedResponse {
                total_threads: u32::try_from(report.total_threads).unwrap_or(u32::MAX),
    ...
    async fn thread_provider_runtime_refresh_response_inner(
        ...
        let status = match status {
            ...
            codex_core::ProviderRuntimeRefreshStatus::Queued => {
                ThreadProviderRuntimeRefreshStatus::Queued
            }
        };

        Ok(ThreadProviderRuntimeRefreshResponse {
    ...
    async fn list_threads_common(
        ...
        let model_provider_filter = match model_providers {
            ...
            // `None` means "no provider filter" so resume and history surfaces
            // keep showing sessions created under other providers.
            None => None,
        };
...
fn core_provider_runtime_refresh_scope(
    scope: Option<ThreadProviderRuntimeRefreshScope>,
) -> codex_core::ProviderRuntimeRefreshScope {
    match scope.unwrap_or(ThreadProviderRuntimeRefreshScope::All) {
        ThreadProviderRuntimeRefreshScope::All => {
            codex_core::ProviderRuntimeRefreshScope::All
        }
        ThreadProviderRuntimeRefreshScope::Console => {
            codex_core::ProviderRuntimeRefreshScope::Console
        }
        ThreadProviderRuntimeRefreshScope::AppServer => {
            codex_core::ProviderRuntimeRefreshScope::AppServer
        }
    }
}
...
fn build_thread_from_snapshot(
    ...
    Thread {
        ...
        cli_version: concat!(env!("CARGO_PKG_VERSION"), "-local3").to_string(),
