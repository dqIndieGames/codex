...
- [Windows Control Plane](#windows-control-plane)
- [Events](#events)
- [Approvals](#approvals)
- [Skills](#skills)
- [Apps](#apps)
...
- `thread/start` — create a new thread; emits `thread/started` (including the current `thread.status`) and auto-subscribes you to turn/item events for that thread. Pass `sessionStartSource: "clear"` when starting a replacement thread after clearing the current session so `SessionStart` hooks receive `source: "clear"` instead of the default `"startup"`.
...
- `thread/providerRuntime/refresh` — reload the user config layer for one loaded thread, keep the current provider id sticky, and refresh only that provider runtime’s `base_url` plus `experimental_bearer_token`. Returns `{ threadId, status }`, where `status` is `applied` after the live runtime has been updated. `queued` remains reserved for any future path that truly defers the refresh until later.
- `thread/providerRuntime/refreshAllLoaded` — run the same provider-runtime refresh flow across every loaded thread in the current app-server instance. Returns `{ totalThreads, appliedThreadIds, queuedThreadIds, failedThreads }`. An instance-level caller can treat `failedThreads = []` as success, including the degenerate `totalThreads = 0` case.
...
On Windows, `codex app-server` also starts a minimal control plane for local instance discovery and bulk provider refresh. This is intentionally separate from the public JSON-RPC transports above; it is not a third general-purpose app-server transport.

- Instance registry: each running app-server writes `$CODEX_HOME/app_servers/<instance_id>.json`.
- Registry fields: `instance_id`, `pid`, `control_endpoint`, `started_at`, `heartbeat_at`.
- Heartbeat cadence: every 5 seconds while the process is alive.
- Normal shutdown: the app-server removes its own registry file before exit.
- Control endpoint: `\\\\.\\pipe\\codex-app-server-<instance_id>`.
- Pipe protocol: single-request / single-response NDJSON.
- Supported pipe operations: `ping`, `refresh_all_loaded_threads`.

The Windows tray tool discovers instances only from that registry directory, cleans stale entries before refresh, and then calls the named pipe control endpoint for each surviving instance.

Default tray delivery:

- Script path: `scripts/windows_app_server_refresh_tray.py`
- Runtime: system Python
- Virtual environment: not required
- Python dependencies: `pystray`, `Pillow`
- Windows APIs: the script talks to named pipes and `MessageBoxW` via `ctypes`, not `pywin32`

Run it directly with system Python:

```powershell
python scripts/windows_app_server_refresh_tray.py
```
