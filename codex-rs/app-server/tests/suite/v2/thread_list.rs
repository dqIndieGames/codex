...
#[tokio::test]
async fn thread_list_without_provider_filter_returns_all_providers() -> Result<()> {
    let codex_home = TempDir::new()?;
    create_minimal_config(codex_home.path())?;
    ...
    assert_eq!(data[0].model_provider, "other_provider");
    assert_eq!(data[1].model_provider, "mock_provider");

    Ok(())
}

#[tokio::test]
async fn thread_list_empty_provider_filter_returns_all_providers() -> Result<()> {
    let codex_home = TempDir::new()?;
    create_minimal_config(codex_home.path())?;
