...
#[test]
fn thread_provider_runtime_refresh_all_loaded_params_round_trip_scope() {
    let console = ThreadProviderRuntimeRefreshAllLoadedParams {
        scope: Some(ThreadProviderRuntimeRefreshScope::Console),
    };
    assert_eq!(
        serde_json::to_value(&console).expect("serialize console refresh scope"),
        json!({ "scope": "console" })
    );
    assert_eq!(
        serde_json::from_value::<ThreadProviderRuntimeRefreshAllLoadedParams>(json!({
            "scope": "appServer"
        }))
        .expect("deserialize app-server refresh scope")
        .scope,
        Some(ThreadProviderRuntimeRefreshScope::AppServer)
    );
    assert_eq!(
        serde_json::from_value::<ThreadProviderRuntimeRefreshAllLoadedParams>(json!({}))
            .expect("deserialize default refresh scope")
            .scope,
        None
    );
}
